...
export default function ProductCarousel2({ products }: { products: any[] }) {
      ...
      <div className="relative overflow-hidden">
        ...
        >
          {pages.map((group, i) => (
            <div
              key={i}
              className="w-full flex gap-0 px-0 shrink-0"
            >
              {group.map((product) => (
                <div
                  key={product.id}
                  className="flex-1 min-w-0 border p-0"
                >
                  <ProductCardOptions product={product} withBtn/>
                </div>
              ))}
            </div>
          ))}
